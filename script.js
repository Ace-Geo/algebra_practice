...
function handleActualMove(from, to, isLocal) {
    ...
    if (isInCheck(currentTurn, boardState)) {
        if (!hasLegalMoves(currentTurn)) { isGameOver = true; status = "CHECKMATE!"; moveNote += "#"; }
        else moveNote += "+";
...
function showSetup() {
    ...
    const draw = () => {
        ...
        <div class="setup-card">
            ...
            ${tab==='create' ? `<div class="input-group"><label>Min | Sec | Inc</label><div class="time-row"><input type="number" id="tM" value="10"><input type="number" id="tS" value="0"><input type="number" id="tI" value="0"></div></div><div class="input-group"><label>Play As</label><select id="pC"><option value="white">White</option><option value="black">Black</option><option value="random">Random</option></select></div>` : ''}
...
socket.on("game-start", (data) => {
    ...
    blackTime = whiteTime; increment = parseInt(data.settings.inc); isInfinite = (whiteTime === 0);
...
socket.on("confirm-settings", (d) => { if(confirm(`Join game by ${d.creatorName}?`)) socket.emit("join-confirmed", { password: currentPassword, name: document.getElementById('uName').value }); });
